...
resource "null_resource" "build_push_image" {
  provisioner "local-exec" {
    command = <<-EOT
      ...
      docker build --platform linux/amd64 -t "europe-docker.pkg.dev/sandbox-bsolano/fastapi-template-repository/fastapi-template" -f Dockerfile.prod .
      docker push "europe-docker.pkg.dev/sandbox-bsolano/fastapi-template-repository/fastapi-template"
    EOT
  }
}
...
resource "google_cloud_run_service" "backend_service" {
  ...
  template {
    spec {
      containers {
        image = "europe-docker.pkg.dev/sandbox-bsolano/fastapi-template-repository/fastapi-template"
      }
    }
    metadata {
      annotations = {
...
resource "google_cloudbuild_trigger" "cloudbuild_trigger" {
  ...
  trigger_template {
    branch_name = "main"
    repo_name   = "bensolano/test-fastapi-generator"
  }
